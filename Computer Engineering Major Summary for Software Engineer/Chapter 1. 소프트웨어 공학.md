# Chapter 1. 소프트웨어 공학

## part 1. 소프트웨어공학 개요

### 1. 소프트웨어 개발에 관한 전반적 흐름

 - 소프트웨어 이해

    - 프로그램 vs 소프트웨어
      	- source code
      	- 산출물 (자료구조, DB구조, 테스트 결과)
      	- 매뉴얼

 - 소프트웨어 분류

    - 정보관리 소프트웨어
      	- 제어 소프트웨어
      	- 임베디드 소프트웨어

 - 소프트웨어 특징

    - 제조가 아닌 개발
      	- 개발자의 역량에 따라 퀄리티가 천차만별
      	- 소모가 아닌 품질 저하

 - 소프트웨어 개발의 문제점

   	- 느린 발전 속도
      	- 새로운 것에 대한 사용자 요구 증가
    - 관리가 필요
       - 비용 / 일정 / 개발자 관리
         	- PMBOK 활용
      	- 개발 과정 복잡
              	- 참여 인력 증가 -> 협업
              	- 개발 기간 증가

   => 기획과 개발, 협업에 대한 어필이 필요

### 2. 소프트웨어 공학을 비롯한 프로젝트 관리

- 소프트웨어 공학의 정의

  - 품질 좋은 소프트웨어를 경제적으로 개발하기 위해
    계획을 세우고 개발하며 유지 및 관리하는
    전 과정에서 공학, 과학 및 수학적 원리와 방법을 적용하여
    필요한 이론과 기술 및 도구들에 관해 연구하는 학문

- 소프트웨어 공학의 목표

  1. 과정의 효율화 -> 생산성 향상
  2. 고품질 소프트웨어 생산 : 사용자의 만족

- 소프트웨어 공학의 특성

  - 공학적 특성

    - 과학적 지식으로 문제 해결
    - 한정된 기간, 비용

  - 소프트웨어 공학만의 특성

    - 소프트웨어 + 공학
    - 개발 과정에 공학적 원리 적용
    - 개발의 어려움 해소
    - 소프트웨어 공학의 목표 실현

  - 소프트웨어 개발 생명주기 (Software Development Life Cycle, **SDLC**)

    - 10단계

    ![SDLC](https://upload.wikimedia.org/wikipedia/commons/thumb/b/bb/Systems_Development_Life_Cycle.jpg/720px-Systems_Development_Life_Cycle.jpg)

    - 5단계
      1. 요구사항 분석
      2. 설계
      3. 개발
      4. 테스트
      5. 운영(유지보수)



## part 2. 소프트웨어 개발 단계

- 프로세스

  - 일이 처리되는 과정, 공정
  - 일을 해결하기 위한 일련의 절차
  - 목적
    - 가이드 : 노하우 전달, 시행착오 감소, 효율화

- 소프트웨어 개발 프로세스

  - task의 집합 + 제약 조건을 포함하는 일련의 activity
    - task : 개발 과정 중의 작은 단위
  - 좁은 의미
    - 절차, 과정, 구조
  - 넓은 의미 (통합적 수단)
    - 좁은 의미 + 도구, 참여자

  - 절차
    1. 계획
    2. 요구분석
    3. 설계
    4. 구현
    5. 테스트
    6. 유지보수

 ### (1) 소프트웨어 프로세스 모델

  - 정의
    - SW를 어떻게 개발할 것인가에 대한 흐름을 체계화
    - 계획 수립부터 최종 폐기까지의 전 과정
    
  - 목적
    - 개발 전 과정을 하나의 프로세스로 정의
    - 주어진 예산, 자원으로 개발 및 관리하는 방법 정의
    
  - 역할
    - 프로젝트의 기본 골격을 구조화
    - 일정, 인력, 금액을 포함한 자원의 사용 계획 수립 및 산정, 분배
    - 의사소통의 기준을 정한다
      - 용어의 표준화
    - 명확한 개발 진행 상황 파악
    - 산출물을 활용, 검토

  - 종류

    1. #### 선형 순차적 모델류

    - 1. **Linear sequential**

      2. **Waterfall (폭포수)**
         ![waterfall](https://upload.wikimedia.org/wikipedia/commons/thumb/e/e2/Waterfall_model.svg/350px-Waterfall_model.svg.png)
       - 요구사항 변화가 적은 프로젝트에 적합
          - 장점
            - 쉬운 관리, 체계적 문서화
          - 단점
            - 각 단계의 완료를 기다려야 함
            - 각 단계의 결과물이 완벽한 수준으로 작성되어야 함
            - 과정 중 고객에게 가시적인 산출물이 없다

      3. **Classic life cycle**

      4. **V model (V 모델**)
         ![vmodel](https://upload.wikimedia.org/wikipedia/commons/thumb/9/96/V-model.JPG/350px-V-model.JPG)

      - 폭포수 모델의 개량형
        - 각 단계에서 테스트 단계를 포함함

    2. #### 진화적 프로세스 모델류

    - 

      1. **Prototype Model (프로토타입 모델)**
         ![prototype](https://lh3.googleusercontent.com/proxy/2yOTVM7nCHVztv7dt3IYLCrW6hoQrMUiiQJ5wSDHbfj1PXa4VG07TgSJq1xvmYBaRRzESoP1_IDZFmnCn4MDi-AVg2PTuSK3twuBLYs)

         - 사용자의 요구를 통해 모형을 제작 후 의사소통

         - 오픈 베타 서비스들이 여기에 해당함

         - 장점

           - 요구사항 충분히 반영
           - 새로운 요구사항 발견

         - 단점

           - 인력 및 비용 산정 어려움
           - 프로토타이핑 과정 관리 어려움

           => 개발 목표 불확실

      2. **Spiral Model (나선형 모델)** 

         - 개발절차
           ![spiralmodel](https://lh3.googleusercontent.com/proxy/R64STkHwDdiI4HgwBWWpXltO-1Umc3RPabvxfQLcEdu1TYEG1vXvtbJWzo_-BF2zm0wTusgJKdCnfkloVGhwV-biLMuoDnIIpaS3jw_Cq0H-XCY89fnWilsq7VqCiBxVYKacHEMkyu7Fa7-DCk9w6FblZ6KaS6k)

         - 프로토타입 모델 + 위험 분석
         - 위험 요소
           - 변경되는 요구사항
           - 팀원 경험 부족
           - 팀워크, 결속력 저하
           - 프로젝트 관리 부족
         - 장점
           - 위험 분석을 통한 돌출 위험 감소
           - 요구사항 충분 반영
         - 단점
           - 프로젝트 기간 연장, 관리 어려움
           - 위험 관리 전문가 필요

    3. #### 단계적 개발 모델류

       - Release (배포) 방법에 따른 분류

       - **점증적 개발 방법** : 범위 증가
         - 주요 기능부터 개발, 범위를 늘려나감

       - **반복적 개발 방법** : 품질 증가
         - 대략적으로 구현하고 보강 및 변경

       1. **Unified Process Model (UP, 통합 프로세스 모델)**

       ![total](https://img1.daumcdn.net/thumb/R720x0.q80/?scode=mtistory2&fname=http%3A%2F%2Fcfile7.uf.tistory.com%2Fimage%2F995C39335A2E039E2DA402)

       - 개발 방법

       ![total](data:image/jpeg;base64,)

         - 절차
           1. 도입 단계
           2. 구체화 단계
           3. 구축 단계
           4. 전이 단계
           5. 도입 / 구체화 / 구축 / 전이 의 공통 과정

       2. **Agile Process Model (애자일 프로세스 모델)**

          - Agile : 날렵한, 민첩한

          - 고객의 요구에 민첩하게 대응, 문제를 그때그때 풀어나가는 방법론

          - 애자일의 기본 가치

            - 프로세스, 도구X 개개인 상호 소통 중심O
            - 문서X, 실행 가능한 프로세스 중심O
            - 협상X, 협력O
            - 계획X, 대응O

          - 개발 방법

            - 반복적 개발, 잦은 출시 목표

            ![agile](https://snowdeer.github.io/assets/2017-02-13-software-development-process/04.jpg)

          - 최근 가장 많이 사용됨
            
            - 페이스북, 카카오톡 등
          - **Scrum Development Process (스크럼 개발 프로세스)**
            - 특징
              - 팀의 개선, 프로젝트 관리
              - 구체적 프로세스는 명확X
              - 팀을 운영하는 효율적 운영 방식
              - 30일 개발 주기
              - Sprint 또는 Iteration
            - 장점
              - 사용자와의 충분한 의견 조율
              - 팀원 간 신속한 협조, 조율
              - 단순하고 실천 지향적
              - 목표, 결과 추정, 변화 시도 가능
            - 단점
              - 일일 스크럼 회의 필요
              - 효율성 및 품질 평가 불가
          - **Extreme Programming (XP)**
            - 의사소통, 피드백, 단순함, 용기, 존중
            - 2주 가량 반복 개발


## (2) 계획 단계

- #### 문제의 정의

- 무엇을 개발할 것인가? 개발의 범위 결정

- #### 타당성 분석

  - 경제적 타당성 : 돈이 되는가
  - 기술적 타당성 : 만들 수 있는가
  - 법적 타당성 : 법적 문제가 없는가

- #### 개발비 산정, 어려움

  - 개발자 능력 차이

  - 개발 프로세스 다양화로 표준화 / 자동화 어려움

  - ##### 개발 비용에 영향을 주는 요소

    - 프로그래머 능력
    - 소프트웨어 복잡도
      - 브룩스의 법칙
        애플리케이션 < 유틸리티 < 시스템 프로그램
      - 개발 비용에 영향을 미침
    - 소프트웨어 크기
      - 인력, 기간, 복잡도 증가
    - 가용 시간
      - 인력 / 자원 증가 -> 개발 기간 단축X
    - 신뢰도 수준 : 높은 신뢰도는 비용 증가
    - 기술 수준 : 고급 언어 사용시 생산성 증가

1. ### 하향식 산정 기법

   >  위에서 슥 보고 판단

   - 전문가 판단 기법
     - 경험 많은 전문가가 산정 : 신뢰도 높음
     - 짧은 시간에 결정해야 하는 경우 용이
     - 단점
       - 수학적 계산법보다 부정확할 가능성
     - ex) **델파이 기법**

2. ### 상향식 산정 기법

   > 하단부터 계산해 올라가며 합산

   - 세부 작업 단위별 비용 산정 후 합산

     - **LOC 기법**
       - 원시 코드 라인 수
     - **Effort Per Task** 기법
       - 각 단계별 노력 ( M/M ) 을 산정

3. ### 수학적 산정 기법

   - **COCOMO 방법**
     - **SW 규모**(LOC) 예측 후 SW 종류에 따라 비용 산정 공식 대입
   - **Putnam 방법**
     - **SW 생명주기** 별 노력 분포 추정
   - **FP (Function Point, 기능 점수) 방법**
     - **기능** 점수를 구한 후 비용 산정

- #### SW 개발 프로젝트의 일정 계획

  - 일정 계획
    - 작업 순서, 소작업 개발 기간, 자원 등

  1. **WBS (Work Breakdown Structure, 작업 분할 구조도)**

     - 활동과 업무를 세분화 
     - 계층 구조로 분류
     - 전체 범위 결정

  2. **PERT (The Program/Project Evaluation and Review Technique)
     / CPM (Critical Path Method)** 
     ![PERTCPM](https://i.ytimg.com/vi/dFTG3ohAcso/maxresdefault.jpg)

     - WBS의 작업물을 네트워크 형태 그래프로 표현
     - 중요한 작업과 여유있는 작업 파악
       => 작업의 우선 순위 결정

  3. **Gantt Chart ( 간트 차트 )**

     ![gantt](https://upload.wikimedia.org/wikipedia/commons/e/e2/GanttChartAnatomy.png)

     - 프로젝트 일정 관리를 위한 바 형태의 도구

- #### 위험관리절차

  - 위험 요소 식별 > 분석 > 수립 > 감시



## (3) 요구 분석

- 용어 정리

  - SW 개발의 목적 : 소프트웨어 고객 만족
  - 고객 만족을 위한 특성
    - 적시성 / 유연성 / 통합
  - 고객 만족의 개발 조건
    - 고품질 / 정해진 기간 / 주어진 예산
  - Requuirement ( 요구 사항 )
    - 사용자 & 개발자 간 합의한 개발 범위 내 시스템이 제공해야 할 기능
  - 요구 분석 명세서
    - 기능적 & 비 기능적 요구 사항 정리 문서

- 요구 분석 과정

  - 사용자 요구 분석 > SW목표수립 > 모델링 > 요구분석명세서

  - 사용자 요구 분석 절차

    1. 자료 수집 : 현황 파악, 담당자 인터뷰 등
    2. 요구 사항 도출 : 자료 정리
    3. 문서화 : 요구 분석 명세서 작성
    4. 검증 : 요구 분석 명세서 검토

  - 요구 사항 분류

    1. 분류
       - 기능적 요구사항
       - 비 기능적 요구사항
         - 품질, 제약사항

    2. 분류
       - 사용자 요구사항
       - 시스템 요구사항

- 품질

  - 신뢰성 : 믿고 사용할 수 있는 정도
  - 신뢰도 측정 : 전체 이용 시간 중 정상 작동 시간
    - MTBF(Mean Time Between Failure, 고장 간 평균 시간)
    - Availability ( 이용 가능성 ) : 주어진 시점에서 프로그램이 작동될 가능성
  - 성능
    - 응답 시간, 데이터 처리량 만족 정도
  - 보안성
    - 사용자 인증 신뢰도
  - 안전성
    - 소프트웨어 오류로 인명 피해 발생X 정도
    - 신호 제어, 발전소 제어, 자율 주행 등
  - 사용성
    - 사용 편의성
    - UX

- 소프트웨어 개발에서의 모델링

  - 여러 관점의 모델 사용

  - **UML 다이어그램 이용**

    - 장점
      - 개발자 & 클라이언트 간 이해도 향상
      - 유지보수 용이
    - 단점
      - 과도한 문서화, 일정 지연
      - 형식적 산출물화

  - 모델링 언어

    - 기호, 표기법, 도구

    - 개발 방법론에 따른 모델링 언어

      - 구조적 방법론
        DFD(Fata Flow Diagram), Data Dictionary, 프로세서 명세서
        ![dfd](https://lh3.googleusercontent.com/proxy/UTACnoe8n_4zVGPUzYlSygMp1upUGNg4Er1PevtPAAZwbeAGT1bTncrnvO03KgOxpubmnflPuB49Hp89COoWq7PfcS8enK74B87GD_r_5SIoaSSW-68mHQLBZ4GYNOEX-iEaSuvBH8fQQi6LSmVNmKI)
        사용자 입장의 순서도

      - 정보공학 방법론
        ERD(Entity-Relationship Diagram) : DataBase 표현
        ![ERD](https://cdn.visual-paradigm.com/guide/data-modeling/what-is-erd/17-dfd-data-store-modeled-by-erd.png)

      - 객체지향 방법론
        UML ( Unified Modeling Language , 통합 모델링 언어) 표기법

        ex) Use-case Diagram
        ![usecase](https://upload.wikimedia.org/wikipedia/commons/thumb/1/1d/Use_case_restaurant_model.svg/1200px-Use_case_restaurant_model.svg.png)


## part. 3 소프트웨어 개발단계 2 (설계 / 구현 / 테스트)

## (1) 설계

- 분석 단계 : WHAT?

- 설계 단계 : HOW?

  - 비 기능적 요구 사항, 제약 사항 고려
  - 운영체제, 미들웨어, 프레임워크 등 플랫폼 결정
  - 어떻게 구축할 것인가
  - 정량적 평가 기준 명시

- ### 설계의 종류

  1. 상위 설계

     > 전반적인 흐름과 구성에 대한 설계

     - 아키텍쳐 설계
     - 데이터 설계
     - 시스템 분할
     - 인터페이스 정의
     - UI 설계

  2. 하위 설계

     > 각 기능, 모듈별 설계

     - 각 모듈을 알고리즘 형태로 표현
     - 인터페이스 설명, 자료구조, 변수 등 상세 정보

- ### 소프트웨어 아키텍처

  - 아키텍처란 ? *~~많다..~~*

    - 정의 
      - 구성 요소
      - 구성 요소 간 관계
      - 구성 요소 외부 드러나는 속성
      - 구성 요소와 주변 환경 사이 관계
      - 구성 요소 제공 인터페이스
      - 구성 요소 협력 및 조립법
    - 소프트웨어에 대한 전체적 구조
    - 구성 요소( 서브시스템, 컴포넌트 ) 의 구성
    - 인터페이스 간 상호작용
    - 설계, 개발 시 원칙 & 지침

  - 시스템 품질 속성
    가용성 / 변경 용이성 / 성능 / 보안성 / 사용성 / 테스트 용이성

  - 비즈니스 품질 속성
    시장 적시성 / 비용과 이익 / 예상 시스템 수명 / 목표 시장  / 공개 일정 / 기존 시스템과의 통합

    - ex) 한국 공공기관 웹 서비스, 전자정보 프레임워크

    > 아키텍처에는 시스템적 성능 뿐 아니라 비즈니스적 요소도 포함

  - 아키텍처 품질 속성

    - 개념적 무결성 : 일관성
    - 정확성, 완전성 : 사용자 요구 기능 충족
    - 개발 용이성 : 기간 내 완성 가능성, 쉽게 변경 및 작업 가능성

  - 아키텍처 구축 절차

    - 요구 사항 분석
      - 기능, 비기능 요구사항 명세
    - 아키텍처 분석 : 어떻게 구성되어야 하는가
    - 아키텍쳐 설계
      스타일 선택, 후보 도출
    - 검증 및 승인
      아키텍처 상세화, 평가, 승인

  > 아키텍처의 디테일을 아는 것 보다, 왜 필요한지 어떻게 쓰이는지, 
  >
  > 새로운 아키텍처를 얼마나 빨리 이해할 수 있는지가 중요

  1. **Repository Model ( 데이터 중심형 모델 )**

    - 중앙 집중형 저장소, 서브 시스템 접근
    - Repository 변경 시 모든 서브시스템 변경 필요

    ![repo](https://www.cs.odu.edu/~price/cs451/Lectures/05design/arch/arch_ht1x.gif)

  2. **Client - Server Model ( 클라이언트 - 서버 모델 )**

    - 네트워크를 이용한 분산 시스템 형태

    - 데이터 및 처리 기능을 클라이언트, 서버에 분할
    - 클라이언트 : 서버 제공 서비스를 요청하는 서브 시스템

    ![CSModel](data:image/png;base64,)

  3. **Layering Model ( 레이어링 모델 )**

    - 기능을 계층화 배치
    - 상위 계층 : Client
    - 하위 계층 : Server

    ![layer](data:image/png;base64,)

  4. **MVC Model ( Model / View / Controller Model )**
     ![mvc](data:image/png;base64,)

  - 뷰 서브시스템을 필요로 하는 시스템에 적합
  - 장점
    - 느슨한 결합
    - 유지 보수 시 M, V, C 가 분리되어 있으므로 용이
  - 단점
    - 복잡도 증가
    - 속도가 중요한 프로젝트에는 부적합

- ### 디자인 패턴

  - 디자인 패턴이란?

    - 자주 사용하는 설계 형태의 정형화
    - 유형별로 설계 템플릿을 만들어 둠

  - 장점

    - 소프트웨어 구조 파악 용이
    - 개발 시간 단축

  - 단점

    - 객체지향 설계 위주
    - 초기 투자 증가

  - **Gof 디자인 패턴** : 대표적 디자인 패턴들

    > Gang of Four, GoF란 1995년 Addison-Wesley 에서 출간된 『 Design Pattern : Elements of Reusable Object-Oriented Software 』저자 4명을 뜻한다.

    1. Creational Pattern
       객체의 생성 관련
    2. Structural Pattren
       프로그램 구조 관련
    3. Behavioral Pattern
       객체 간 커뮤니케이션 관련

- ### 모듈화

  - 개발에서 큰 문제를 작은 단위로 나누는 것

  - 모듈
    기본적, 논리적, 독립적, 단위 프로그램, ex) 함수, 메서드

  - 특징

    - 독립적 기능
    - 독립적 유일한 이름
    - 독립적 컴파일
    - 모듈 간 호출 가능
    - 다른 프로그램의 모듈 호출 가능

  - 모듈화의 형태

    - 라이브러리 함수, 그래픽 함수
      JavaScript 의 jQuery

  - 장점

    - 분할 정복 원리 적용
    - 복잡도 감소
    - 유지보수 용이
    - 오류 최소화
    - 재사용성

  - **응집도**

    >  어떻게 나눠서 어떻게 만들어가는가

    기능적, 순차적, 교환적, 절차적, 시간적, 논리적, 우연적 응집

  - **결합도**

    >  모듈간 어떻게 결합하는가

    데이터(데이터만), 스탬프(필요없는 데이터까지), 제어(제어 플래그를 매개변수로 사용 -> 간섭), 공통(공통 전역변수 사용), 내용 결합(인터페이스X 직접 접근)

  - 바람직한 설계

    - 낮은 결합도, 높은 응집도
    - 유지보수 용이

- ### 방법론

  - **프로세스 지향 방법**
    - 처리순서를 절차적 구조화
    - DFD ( Data Flow Diagram ) 대표적
    - 기능 중심, 필요한 데이터 참조 형태
  - **데이터 지향 방법**
    - 데이터 중심, 구조화
    - ERD ( Entity - Relation Diagram ) 대표적
  - 프로세스, 데이터 지향 방법의 문제
    - 각각을 별개의 것으로 파악해 변경에 취약
    - 복잡도 증가
    - 변경 시 구조 파악 필요
    - 재사용성 감소
  - **객체 지향 방법 (Object - Oriented Approach)**
    - 프로세스, 데이터 지향 방법 문제 해결
    - 객체 중심 개발
    - 데이터(속성) + 메서드(함수) = 객체
    - 특징
      - 사람의 생각과 유사
      - 시스템은 객체들의 모임
      - 확장성, 재사요엉
      - 추상화 -> 생산성, 품질 증가

## (2) 구현

- 프로그래밍 언어 : Java, C/C++

- 코딩 규칙 : 높은 가독성, 간결하고 명확한 코딩

  > 개발자 간 커뮤니케이션 용이

## (3) 테스트

- IEEE
  예상 결과와 실제 결과의 차이를 검사 및 평가하는 작업
- Zoha Manna : "테스트가 시스템을 증명할 수 없다"
- Dahl, Dijkstra, Hoare : "테스트는 결함을 보일 뿐 무결을 증명할 수 없다"
- 소프트웨어 테스트란?
  - 존재하지만 드러나지 않는 오류를 발견할 목적
  - 정상적 실행을 위한 작업
  - 완전하고 정확한 증명은 불가

- 목표
  - 작은 의미
    - 남아있는 오류 발견, 결함 예방
  - 큰 의미
    - 고객 만족, 신뢰성
- 용어 정리
  - 오류
    개발자의 실수, 결함의 원인
  - 결함
    오류에 의한 프로그램의 불완전성, 고장의 원인
  - 고장
    시스템이 요구 사항대로 작동하지 않는 것
- 분류
  - 시각에 따른 테스트
    - 확인 테스트 (Verification Test)
      - 개발자의 시각
    - 검증 테스트 (Validation Test)
      - 사용자의 시각
  - 사용 목적에 따른 테스트
    - 운영 목적 적합성 테스트
    - 수정 용이성 테스트
    - 운영 지원 용이성 테스트
  - 프로그램 실행 여부에 따른 테스트
    - 정적 테스트 : 실행X, 코드 리뷰
    - 동적 테스트 : 실행O 하며 테스트

## part. 4 소프트웨어 개발단계 ( 품질 / 프로젝트 관리 )

## (1) 품질

- 소프트웨어 품질이란?

  - 사용자의 요구에 부합하는 정도
  - 개발자 관점에서 결합이 없는 정도

- 품질의 목표

  >  말 그대로 잘 만들어진 프로그램을 위한 평가 척도

  - 정확성, 신뢰성, 효율성, 무결성, 접근사용성, 유지보수 용이성, 테스트 용이성, 유연성, 이식성, 재사용성, 상호운용성

  - 서비스마다 우선 순위는 달라질 수 있다.

    > 지원 회사들의 서비스의 우선 순위를 매겨보는 것도 재미있을 듯

- 품질의 특성

  - McCall(맥콜) : "제품 수정 / 변환 / 운영"
  - Garvin(가빈) : "성능, 품질, 신뢰성, 일치성, 내구성, 서비스, 미적, 지각"
  - Boehm(보헴) : "이식성, 신뢰성, 효율성, 인간공학성, 테스트성, 이해성, 수정성"

  > 소프트웨어 설계 단계에서 품질 요소를 정의하는 것은 설계자의 관점과 서비스에 따라 달라질 수 있다는 것을 이해한다

- 품질 평가 표준 모델

  - 제품 품질 특성 평가
  - 프로세스 품질 특성 평가
  - CMMI ( Capability Maturity Model Integration )
    조직의 프로세스에 대한 가이드, 기준
    - C (능력) : 개발 목표 달성 능력
    - M (성숙도) : 객관적, 정량적 근거로 프로세스 측정, 지속적 개선
    - M (모델) : 프로세스를 감사(audit) 하는 의미
    - I (통합) : 소프트웨어 개발 생명주기 각 단계들의 통합된 모델
  - SPICE (ISO 15504) 모델
    - 소프트웨어 프로세스 평가를 위한 프레임워크 제공
    - 품질 표준, 규격화 제공

> 품질에서 가장 중요한 것은 사람, 조직, 조직의 관리
>
> 소프트웨어 공학의 학습이란 :
> 	프로젝트 개발 과정 중 어떤 것들을 중점적으로 고려해야 하는지 아는 것



## (2) 프로젝트 관리

- 프로젝트란 ?

  - 정의
    - 유일한 제품이나 서비스를 만들기 위해 일정 기간 수행하는 작업
  - 특징
    - 한시성 : 시작과 끝이 존재
    - 유일성 : 유일한 산출물
    - 일시성 : 프로젝트 종료 시 참여 인력 해체
    - 한정성 : 프로젝트 종료 시 사용된 자원은 돌려 둠

- PMBOK (Project Management Body of Knowledge, 프로젝트 관리의 총체적 지식에 대한 가이드)

  - 5가지 프로세스 그룹

    1. 시작 Initiating 그룹
    2. 기획 Planning 그룹
    3. 실행 Executing 그룹
    4. 통제 Controlling 그룹
    5. 종료 Closing 그룹

  - 9가지 관점

    통합, 범위, 시간, 비용, 품질, 인적 자원, 통신, 위험, 조달

  > 프로젝트 과정 시 단지 코딩만 해서 될 것이 아니라 과정들을 평가하는 기준이 필요하다는 것.

- 정보 시스템 감리

  - 정보 시스템에 대해 제 3자적 관점에서 종합적으로 점검하고 문제점을 개선하도록 하는 것

  - '전자정부법' 에 명시

  - 목적

    > 프로젝트 계획 단계의 타당성 분석과 관계있는 듯

    - 효과성 확보
    - 효율성 확보
    - 안정성 확보
    - 법적 요건 준수 확인

  - 감리대상

    - 정보시스템의 구축 및 운영 등
    - 규모 : 사업이 5억원 이상의 정보시스템 구축 사업
    - 특성 : 대국민 서비스/민원 등, 다수기관 공동 구축/사용

## part 5. IT 기술용어 정리

> 각 회사들이 사용하는 용어들의 대략적 흐름

- **데이터 마이닝 (Data Mininig)**

  - 대용량 데이터로부터 요구사항에 맞게 의미있는 정보와 지식을 분석 & 추출
  - 등장 배경
    - 기술적 측면
      - 프로세싱, 자료 저장, 기계학습 기술의 발전
    - 비즈니스 측면
      - Mass 마케팅 -> Target 마케팅 패러다임 변화
      - 새로운 비즈니스 창출 / 개선 / 활용

- **웹 마이닝 ( Web Mining )**

  - 웹 환경에서 얻은 문서, 정보로 행위, 패턴 등의 정보를 마케팅 및 의사결정에 활용하기 위한 마이닝 기법
  - 데이터 마이닝과의 차이
    - 클릭 뿐 아니라 스크롤 및 체류시간에 대한 데이터까지도 수집

- **BCP ( Business Continuity Planning )**

  - 천재지변 등 재해 발생 시 시스템 복구, 데이터 복원 등을 넘어 기업의 비즈니스 연속성을 보장할 수 있는 체계
  - BCP 프레임워크 내 주요 계획
    - 재해 예방
    - 대응 및 복구
    - 유지보수
    - 모의훈련

- **BCM ( Business Continuity Management )**

  - 예기치 않은 상황에서도 비즈니스를 지속적으로 운영하기 위해, 연속성을 보장할 수 있는 경영
  - BCP의 상위 개념

- **DRS ( Disaster Recovery System )**

  - 정보 시스템의 비상 대비체계 유지, 복구계획 수립을 통한 업무 연속성 유지 체제

  - ex) 최근 KT 화재

- **BIA ( Business Impact Analysis )**

  - 재해 발생시 가능한 위험을 정의하고 정량적 / 정성적 분석을 통해 우선순위 도출

> 재해, 재난, 대규모 해킹 발생시 대처가 최근 이슈



## 정리

- 소프트웨어 공학
  - 소프트웨어를 어떻게 기획하고 개발하고 운영/관리 하는 방법론
  - 실무에서 협업을 위해 이해가 필요
  - 내가 작업한 프로젝트 결과물을 설명할 때
    - 단순하게 뭘 만들었다가 아니라
    - 소프트웨어 공학적인 다양한 측면에서 설명할 수 있다
  - 과정별 산출물을 통해 작업 과정을 문서화 가능

## part 6. 면접 기출문제 정리

1. 왜 소프트웨어 공학이 필요할까요?

<details>
    <summary>예시 답안</summary>
    <div markdown="1">
        집 짓는 것에 비유하자면 단지 머릿 속으로 구상해서 짓기 보다는 기존에 지어왔던 경험자들의 방법론을 따라가면 발생가능한 문제점과 위험요소들로부터 보다 효율적으로 실수없이 개발할 수 있게 되기 때문.
        협업 시 필요한 커뮤니케이션을 위해서, 기존에 구축된 시스템을 이해하기 위해서.
    </div>
</details>


2. 소프트웨어 개발 경험을 설명해보시오.
   프로젝트 내용, 기술 스택으로만 말하는게 아니라...

<details>
    <summary>예시 답안</summary>
    <div markdown="1">
    	소프트웨어 공학적 관점에서 한번 작성해보자....
    	어떠한 목적, 요구사항으로, 과정에 따라, 문제점이 발생했는지
    	어떠한 개발 방법론을 사용하여, 디자인 패턴을 사용하여,
    	면접하는 회사의 특성에 관련지어서,
    </div>
</details>

3. Agile 방법론에 대해 설명하시오

<details>
    <summary>예시 답안</summary>
    <div markdown="1">
        실제 뭔가를 개발하는데 관련된 방법론이라기 보다는
        고객의 요구사항을 빠르게 받아들여 제품에 반영시키기 위한 조직 관리에 관련된 방법론 Sprint, Scrum 등
        고객의 요구사항을 어떻게 받아들여 적용시킬 수 있는지 알고 있는지에 대한 질문
    </div>
</details>


4. Lean 개발 방법론에 대해서 설명하시오

<details>
    <summary>예시 답안</summary>
    <div markdown="1">
        고객의 요구사항을 반영해서 처리하기 위해 어떻게 프로세스를 정의하고 관리하는지에 대한 방법론
        스타트업이 개발하려고 할때, 처음부터 고객의 요구사항을 알 수 없기 때문에 프로토타입을 시장에 공개하고 피드백을 받아 다시 업데이트를 반복하는 방식
        Agile 이나 Lean 은 빠르게 변화하는 고객의 요구사항에 대응하기 위해사용하는 개발 방식
    </div>
</details>

